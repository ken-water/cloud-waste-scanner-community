...
async fn seed_scan_rules(pool: &Pool<Sqlite>) -> Result<(), String> {
    ...
    let rules = vec![
        ...
        (
            "aws_eks_nodegroup_floor_risk",
            "aws",
            "EKS NodeGroup Floor Risk",
            "Detect EKS node groups with high baseline floor and low average utilization.",
            "{}",
        ),
        (
            "aws_eks_orphan_pv",
            "aws",
            "EKS Orphan PV Volumes",
            "Detect unattached Kubernetes-tagged EBS volumes likely left by stale PV/PVC lifecycle.",
            "{}",
        ),
        (
            "aws_eks_orphan_eni",
            "aws",
            "EKS Orphan Network Interfaces",
            "Detect available ENIs with Kubernetes/EKS footprint that are likely stale CNI attachments.",
            "{}",
        ),
        (
            "aws_eks_owner_gap",
            "aws",
            "EKS Ownership Tag Gaps",
            "Detect EKS nodes without owner/team/cost tags, blocking chargeback and lifecycle accountability.",
            "{}",
        ),
        (
            "aws_eks_requests_limits_drift",
            "aws",
            "EKS Requests/Limits Drift Proxy",
            "Detect EKS node groups with heavy instance classes, persistent floor, and low utilization as a proxy for requests/limits imbalance.",
            "{}",
        ),
        // Azure
        (
        ...
        (
            "azure_aks_nodepool_floor_risk",
            "azure",
            "AKS Node Pool Floor Risk",
            "Detect AKS node pools where autoscaling min floor or fixed node count is likely overbuilt.",
            "{}",
        ),
        (
            "azure_aks_orphan_pv",
            "azure",
            "AKS Orphan PV Disks",
            "Detect unattached AKS/Kubernetes-like managed disks for PV/PVC cleanup review.",
            "{}",
        ),
        (
            "azure_aks_orphan_ip",
            "azure",
            "AKS Orphan Public IPs",
            "Detect unattached AKS/Kubernetes-like Public IPs likely left by stale Service LoadBalancer lifecycle.",
            "{}",
        ),
        (
            "azure_aks_owner_gap",
            "azure",
            "AKS Ownership Tag Gaps",
            "Detect AKS node pools without owner/team/cost tags, blocking team chargeback and accountability.",
            "{}",
        ),
        (
            "azure_aks_requests_limits_drift",
            "azure",
            "AKS Requests/Limits Drift Proxy",
            "Detect AKS node pools with large VM classes and narrow autoscaling span as a proxy for requests/limits imbalance.",
            "{}",
        ),
        // GCP
        (
        ...
        (
            "gcp_gke_nodepool_floor_risk",
            "gcp",
            "GKE Node Pool Floor Risk",
            "Detect GKE node pools with high baseline floor in fixed or autoscaling mode.",
            "{}",
        ),
        (
            "gcp_gke_orphan_pv",
            "gcp",
            "GKE Orphan PV Disks",
            "Detect unattached Kubernetes-labeled persistent disks likely left from stale PV/PVC lifecycle.",
            "{}",
        ),
        (
            "gcp_gke_orphan_ip",
            "gcp",
            "GKE Orphan External IPs",
            "Detect reserved unattached external IPs with Kubernetes/GKE labels from stale service lifecycles.",
            "{}",
        ),
        (
            "gcp_gke_owner_gap",
            "gcp",
            "GKE Ownership Label Gaps",
            "Detect GKE node pools without owner/team/cost labels, blocking namespace/team chargeback.",
            "{}",
        ),
        (
            "gcp_gke_requests_limits_drift",
            "gcp",
            "GKE Requests/Limits Drift Proxy",
            "Detect GKE node pools with large machine classes and narrow autoscaling span as a proxy for requests/limits imbalance.",
            "{}",
        ),
        // Alibaba
        (
