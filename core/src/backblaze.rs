...
impl BackblazeScanner {
    ...
    fn normalize_endpoint(raw: &str, region: &str) -> String {
        ...
        if endpoint.is_empty() {
            return format!("https://s3.{}.backblazeb2.com", region);
        }
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!(
                "Backblaze B2 access key and secret key are required"
            ));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Backblaze B2 auth failed: {}", e))?;
        Ok(())
    }
        ...
        for bucket in buckets.buckets() {
            ...
            if is_empty {
                wastes.push(WastedResource {
                    id: name,
                    provider: "Backblaze B2".to_string(),
...
#[async_trait]
impl CloudProvider for BackblazeScanner {
