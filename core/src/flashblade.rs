...
impl FlashbladeScanner {
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!(
                "Pure Storage FlashBlade access key and secret key are required"
            ));
        }
        if self.endpoint.is_empty() {
            return Err(anyhow!("Pure Storage FlashBlade endpoint is required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Pure Storage FlashBlade auth failed: {}", e))?;
        Ok(())
    }
        ...
        for bucket in buckets.buckets() {
            ...
            if is_empty {
                wastes.push(WastedResource {
                    id: name,
                    provider: "Pure Storage FlashBlade".to_string(),
...
#[async_trait]
impl CloudProvider for FlashbladeScanner {
