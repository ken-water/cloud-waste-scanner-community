...
impl HcpScanner {
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!(
                "Hitachi HCP access key and secret key are required"
            ));
        }
        if self.endpoint.is_empty() {
            return Err(anyhow!("Hitachi HCP endpoint is required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Hitachi HCP auth failed: {}", e))?;
        Ok(())
    }
        ...
        for bucket in buckets.buckets() {
            ...
            if is_empty {
                wastes.push(WastedResource {
                    id: name,
                    provider: "Hitachi HCP".to_string(),
...
#[async_trait]
impl CloudProvider for HcpScanner {
