...
impl QumuloScanner {
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!("Qumulo access key and secret key are required"));
        }
        if self.endpoint.is_empty() {
            return Err(anyhow!("Qumulo endpoint is required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Qumulo auth failed: {}", e))?;
        Ok(())
    }
...
#[async_trait]
impl CloudProvider for QumuloScanner {
