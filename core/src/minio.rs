...
impl MinioScanner {
    ...
    fn normalize_endpoint(raw: &str, _region: &str) -> String {
        ...
        if endpoint.is_empty() {
            return "http://localhost:9000".to_string();
        }
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!("MinIO access key and secret key are required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("MinIO auth failed: {}", e))?;
        Ok(())
    }
...
#[async_trait]
impl CloudProvider for MinioScanner {
