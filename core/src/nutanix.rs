...
impl NutanixScanner {
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!(
                "Nutanix Objects access key and secret key are required"
            ));
        }
        if self.endpoint.is_empty() {
            return Err(anyhow!("Nutanix Objects endpoint is required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Nutanix Objects auth failed: {}", e))?;
        Ok(())
    }
        ...
        for bucket in buckets.buckets() {
            ...
            if is_empty {
                wastes.push(WastedResource {
                    id: name,
                    provider: "Nutanix Objects".to_string(),
...
#[async_trait]
impl CloudProvider for NutanixScanner {
