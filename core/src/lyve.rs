...
impl LyveScanner {
    ...
    fn normalize_endpoint(raw: &str, region: &str) -> String {
        ...
        if endpoint.is_empty() {
            return format!("https://s3.{}.lyvecloud.seagate.com", region);
        }
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!(
                "Seagate Lyve Cloud access key and secret key are required"
            ));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Seagate Lyve Cloud auth failed: {}", e))?;
        Ok(())
    }
        ...
        for bucket in buckets.buckets() {
            ...
            if is_empty {
                wastes.push(WastedResource {
                    id: name,
                    provider: "Seagate Lyve Cloud".to_string(),
...
#[async_trait]
impl CloudProvider for LyveScanner {
