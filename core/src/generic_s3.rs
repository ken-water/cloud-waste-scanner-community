...
impl GenericS3Scanner {
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!(
                "Generic S3-Compatible access key and secret key are required"
            ));
        }
        if self.endpoint.is_empty() {
            return Err(anyhow!("Generic S3-Compatible endpoint is required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Generic S3-Compatible auth failed: {}", e))?;
        Ok(())
    }
        ...
        for bucket in buckets.buckets() {
            ...
            if is_empty {
                wastes.push(WastedResource {
                    id: name,
                    provider: "Generic S3-Compatible".to_string(),
...
#[async_trait]
impl CloudProvider for GenericS3Scanner {
...
mod tests {
    ...
    #[test]
    fn normalize_endpoint_handles_scheme_and_trailing_slash() {
        assert_eq!(
            GenericS3Scanner::normalize_endpoint("s3.example.com/", "us-east-1"),
            "https://s3.example.com"
        );
        assert_eq!(
            GenericS3Scanner::normalize_endpoint("http://s3.example.com", "us-east-1"),
            "http://s3.example.com"
        );
        assert_eq!(GenericS3Scanner::normalize_endpoint("   ", "us-east-1"), "");
    }

    #[test]
    fn constructor_defaults_region_and_normalizes_credentials() {
        let scanner = GenericS3Scanner::new(" ak ", " sk ", " ", "s3.example.com/");
        assert_eq!(scanner.region, "us-east-1");
        assert_eq!(scanner.access_key, "ak");
        assert_eq!(scanner.secret_key, "sk");
        assert_eq!(scanner.endpoint, "https://s3.example.com");
    }

    #[test]
    fn s3_client_validates_required_credentials_and_endpoint() {
        let missing_keys = GenericS3Scanner::new("", "", "us-east-1", "s3.example.com");
        assert!(missing_keys.s3_client().is_err());

        let missing_endpoint = GenericS3Scanner::new("ak", "sk", "us-east-1", "");
        assert!(missing_endpoint.s3_client().is_err());
    }
}
