...
impl GreenlakeScanner {
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!(
                "HPE GreenLake access key and secret key are required"
            ));
        }
        if self.endpoint.is_empty() {
            return Err(anyhow!("HPE GreenLake endpoint is required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("HPE GreenLake auth failed: {}", e))?;
        Ok(())
    }
        ...
        for bucket in buckets.buckets() {
            ...
            if is_empty {
                wastes.push(WastedResource {
                    id: name,
                    provider: "HPE GreenLake".to_string(),
...
#[async_trait]
impl CloudProvider for GreenlakeScanner {
