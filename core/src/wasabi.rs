...
impl WasabiScanner {
    ...
    fn normalize_endpoint(raw: &str, region: &str) -> String {
        ...
        if endpoint.is_empty() {
            return format!("https://s3.{}.wasabisys.com", region);
        }
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!("Wasabi access key and secret key are required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Wasabi auth failed: {}", e))?;
        Ok(())
    }
...
#[async_trait]
impl CloudProvider for WasabiScanner {
