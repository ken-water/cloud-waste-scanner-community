...
impl StorjScanner {
    ...
    fn normalize_endpoint(raw: &str, _region: &str) -> String {
        ...
        if endpoint.is_empty() {
            return "https://gateway.storjshare.io".to_string();
        }
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!("Storj DCS access key and secret key are required"));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("Storj DCS auth failed: {}", e))?;
        Ok(())
    }
...
#[async_trait]
impl CloudProvider for StorjScanner {
