...
impl RackspaceScanner {
    ...
    fn normalize_endpoint(raw: &str) -> String {
        ...
        if endpoint.is_empty() {
            return "https://dfw.servers.api.rackspacecloud.com/v2".to_string();
        }
    ...
    async fn request_json(&self, path: &str) -> Result<Value> {
        if self.token.is_empty() {
            return Err(anyhow!("Rackspace API token is required"));
        }
        ...
        serde_json::from_str(&text).map_err(|e| anyhow!("Invalid Rackspace JSON: {}", e))
    }
    ...
    async fn request_any_json(&self, paths: &[String]) -> Result<Value> {
        ...
        Err(last_err.unwrap_or_else(|| anyhow!("Rackspace API request failed")))
    }
    ...
    pub async fn scan_volumes(&self) -> Result<Vec<WastedResource>> {
        ...
        for volume in Self::extract_items(
            ...
            wastes.push(WastedResource {
                ...
                details: format!(
                    "Unattached Rackspace volume: {} ({:.0} GB)",
                    if name.is_empty() { id } else { name },
                    normalized
                ),
    ...
    pub async fn scan_ips(&self) -> Result<Vec<WastedResource>> {
        ...
        for ip in Self::extract_items(
            ...
            wastes.push(WastedResource {
                ...
                details: "Unassigned Rackspace floating IP".to_string(),
...
#[async_trait]
impl CloudProvider for RackspaceScanner {
...
#[cfg(test)]
mod tests {
    use super::RackspaceScanner;
    use serde_json::json;

    #[test]
    fn normalize_endpoint_and_scoped_paths_are_stable() {
        let scanner =
            RackspaceScanner::new("token", "dfw.servers.api.rackspacecloud.com", "proj-a");
        assert_eq!(
            RackspaceScanner::normalize_endpoint("dfw.servers.api.rackspacecloud.com"),
            "https://dfw.servers.api.rackspacecloud.com/v2"
        );
        assert_eq!(
            RackspaceScanner::normalize_endpoint("https://dfw.servers.api.rackspacecloud.com/v2/"),
            "https://dfw.servers.api.rackspacecloud.com/v2"
        );
        let paths = scanner.scoped_paths(&["servers", "/images"]);
        assert!(paths.contains(&"/proj-a/servers".to_string()));
        assert!(paths.contains(&"/servers".to_string()));
        assert!(paths.contains(&"/proj-a/images".to_string()));
        assert!(paths.contains(&"/images".to_string()));
    }

    #[test]
    fn value_extractors_and_size_helpers_cover_variants() {
        let payload = json!({
            "flavor": {"name": "general1-2", "vcpus": "4"},
            "size": "2147483648",
            "metadata": {"name": "demo"}
        });
        assert_eq!(
            RackspaceScanner::str_field(&payload, &["metadata.name"]),
            "demo"
        );
        assert_eq!(
            RackspaceScanner::parse_f64(&payload, &["flavor.vcpus"]),
            Some(4.0)
        );
        assert_eq!(
            RackspaceScanner::parse_f64(&payload, &["size"]),
            Some(2_147_483_648.0)
        );
        assert!((RackspaceScanner::bytes_to_gb(2_147_483_648.0) - 2.0).abs() < f64::EPSILON);
    }

    #[test]
    fn extract_items_and_parse_time_support_multi_shape_inputs() {
        let payload = json!({
            "servers": [{"id":"s1"}],
            "data": {"items": [{"id":"s2"}]}
        });
        let items = RackspaceScanner::extract_items(&payload, &["servers", "data"], &["items"]);
        assert_eq!(items.len(), 2);

        assert!(RackspaceScanner::parse_time("2026-03-01T10:00:00Z").is_some());
        assert!(RackspaceScanner::parse_time("not-a-time").is_none());
        assert!(RackspaceScanner::parse_time("").is_none());
    }
}
