...
impl CivoScanner {
    ...
    fn normalize_endpoint(raw: &str) -> String {
        ...
        if endpoint.is_empty() {
            return "https://api.civo.com/v2".to_string();
        }
        ...
        format!("{}/v2", value.trim_end_matches('/'))
    }

    async fn request_json(&self, path: &str) -> Result<Value> {
        if self.token.is_empty() {
            return Err(anyhow!("Civo API token is required"));
        }
        ...
        serde_json::from_str(&text).map_err(|e| anyhow!("Invalid Civo JSON: {}", e))
    }
    ...
    async fn request_any_json(&self, paths: &[&str]) -> Result<Value> {
        ...
        Err(last_err.unwrap_or_else(|| anyhow!("Civo API request failed")))
    }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        self.request_any_json(&[
            "/instances?per_page=1",
            "/instances",
            "/kubernetes/clusters?per_page=1",
        ])
        .await?;
        Ok(())
    }
    ...
    pub async fn scan_instances(&self) -> Result<Vec<WastedResource>> {
        let payload = self
            .request_any_json(&["/instances?per_page=200", "/instances"])
            .await?;
        ...
        for instance in Self::extract_items(
            ...
            let id = Self::str_field(&instance, &["id", "instance_id"]);
            if id.is_empty() {
                continue;
            }

            let name = Self::str_field(&instance, &["hostname", "name"]);
            let region = Self::str_field(&instance, &["region", "zone"]);
            let size = Self::str_field(&instance, &["size", "instance_type"]);

            wastes.push(WastedResource {
                id: id.clone(),
                provider: "Civo".to_string(),
                ...
                details: format!(
                    "Stopped Civo instance: {} ({})",
                    if name.is_empty() { id } else { name },
                    if size.is_empty() { "unknown" } else { &size }
                ),
    ...
    pub async fn scan_volumes(&self) -> Result<Vec<WastedResource>> {
        ...
        for volume in Self::extract_items(
            ...
            let attached_to =
                Self::str_field(&volume, &["instance_id", "server_id", "attached_to"]);
            ...
            let region = Self::str_field(&volume, &["region", "zone"]);
            let size = Self::parse_f64(&volume, &["size_gb", "size", "capacity"]).unwrap_or(0.0);
    ...
    pub async fn scan_ips(&self) -> Result<Vec<WastedResource>> {
        ...
        for ip in Self::extract_items(
            &payload,
            &["ips", "reserved_ips", "result", "data"],
            &["ips", "reserved_ips", "items"],
        ) {
            let attached_to = Self::str_field(&ip, &["instance_id", "server_id", "attached_to"]);
            if !attached_to.is_empty() {
                continue;
            }

            let id = Self::str_field(&ip, &["ip", "address", "id"]);
            if id.is_empty() {
                continue;
            }

            let region = Self::str_field(&ip, &["region", "zone"]);
            ...
            wastes.push(WastedResource {
                ...
                details: "Unassigned Civo reserved/public IP".to_string(),
    ...
    pub async fn scan_snapshots(&self) -> Result<Vec<WastedResource>> {
        let payload = self
            .request_any_json(&[
                ...
                "/volume_snapshots?per_page=200",
                "/backups?per_page=200",
            ])
            .await?;
        ...
        for snapshot in Self::extract_items(
            ...
            let region = Self::str_field(&snapshot, &["region", "zone"]);
            let size = Self::parse_f64(&snapshot, &["size_gb", "size", "capacity"]).unwrap_or(0.0);
            let normalized = if size <= 0.0 { 20.0 } else { size };
...
#[async_trait]
impl CloudProvider for CivoScanner {
