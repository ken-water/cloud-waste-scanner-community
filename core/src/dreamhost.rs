...
impl DreamhostScanner {
    ...
    fn normalize_endpoint(raw: &str, region: &str) -> String {
        let endpoint = raw.trim();
        if endpoint.is_empty() {
            return format!("https://objects-{}.dream.io", region);
        }
    ...
    fn s3_client(&self) -> Result<S3Client> {
        if self.access_key.is_empty() || self.secret_key.is_empty() {
            return Err(anyhow!(
                "DreamHost DreamObjects access key and secret key are required"
            ));
        }
    ...
    pub async fn check_auth(&self) -> Result<()> {
        ...
        client
            ...
            .send()
            .await
            .map_err(|e| anyhow!("DreamHost DreamObjects auth failed: {}", e))?;
        Ok(())
    }
        ...
        for bucket in buckets.buckets() {
            ...
            if is_empty {
                wastes.push(WastedResource {
                    id: name,
                    provider: "DreamHost DreamObjects".to_string(),
...
#[async_trait]
impl CloudProvider for DreamhostScanner {
